...
def load_models():
    models_list = []
    for i in range(1, 6):
        model = models.load_model(f'model_node_{i}.keras')
...
def save_aggregated_model(aggregated_weights):
    ...
    model = models.load_model('model.keras')  # Load the original architecture (no weights)
    
    # Set the aggregated weights to the model
    model.set_weights(aggregated_weights)
    
    # Save the aggregated model
    model.save('aggregated_model.keras')
    print("Aggregated model saved as 'aggregated_model.keras'.")

# Function to evaluate the aggregated model
def evaluate_aggregated_model():
    # Load the aggregated model
    model = models.load_model('aggregated_model.keras')
